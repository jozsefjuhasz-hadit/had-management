...
> Items flagged by the tech-lead agent while writing TSDs. These are implementation risks, future limitations, or decisions that carry known trade-offs that could require revisiting.

| # | Item | Severity | Source TSD | Notes |
|---|------|----------|-----------|-------|
| TD-1 | Vercel HTTP outbound connectivity to MNB SOAP unverified | HIGH | TSD-reports (ADR-4) | `http://www.mnb.hu/arfolyamok.asmx` uses plain HTTP. Vercel may block non-HTTPS outbound calls. Must be verified in an infrastructure spike before launch. If blocked, a lightweight HTTPS proxy (Cloudflare Worker or similar) is required. Decision documented in ADR-4. |
| TD-2 | HAD Agent seed script is a hard deployment prerequisite | HIGH | TSD-auth, TSD-deals, TSD-mcp-tools (ADR-3) | The `scripts/seed-agent-user.ts` script must be run at initial deployment to create the AI agent's users table record. If skipped, all MCP write operations will fail with FK violations. Must be included in the deployment runbook and CI/CD initialization steps. |
| TD-3 | Exchange rate HUF drift in historical reports | MEDIUM | TSD-reports (ADR-9) | All historical EUR/USD deal values are displayed using the *current* rate, not the rate at time of deal creation. The "2024 actual revenue" figure will change daily with the rate. Documented as a known beta limitation. Post-MVP: add `rate_at_creation` fields to deals table. |
| TD-4 | MCP_API_TOKEN long-lived token is a security risk if leaked | MEDIUM | TSD-mcp-tools (ADR-5) | The MCP server authenticates using a long-lived bearer token stored in the local `.env`. If this token is committed to version control or leaked, it provides full CRM write access as the AI agent. `.env` must be in `.gitignore`; token rotation procedure must be documented. |
| TD-5 | Stage history entries from direct DB migrations will have null actor | MEDIUM | TSD-deals (ADR-8) | The shared `stageTransition()` service function is the only correct path for stage changes. Any direct DB UPDATE to the `stage` field (e.g., in a migration script or manual correction) will bypass stage history logging. Document this constraint prominently in the schema file and deployment runbook. |
| TD-6 | CSV import referred_by column strategy undefined | LOW | TSD-contacts (ADR-7) | TSD-contacts open question: should the `referred_by` column in CSV import also use email/name matching? Currently undefined. If not resolved before implementation, `referred_by` will be silently null on all CSV imports. |
| TD-7 | Activity pagination UX decision deferred | LOW | TSD-deals | Whether "load more" activities on Deal Detail is infinite scroll or paginator is not specified. Recommend "Load more" button. Must be resolved before implementation to avoid UI rework. |
| TD-8 | `search_crm` archived/closed entity inclusion undefined | LOW | TSD-mcp-tools | Whether archived contacts and won/lost deals appear in `search_crm` results is unresolved. Recommend: exclude archived contacts, include closed deals. Pending confirmation. |
